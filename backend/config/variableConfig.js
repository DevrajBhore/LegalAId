...
export const VARIABLE_CONFIG = {
  ...
  COMMON: {
    // Which side of this agreement the user is on.
    //
    // Nothing in the system represented this, and a surprising amount depends
    // on it. Every objective on the "what should this protect you from" step is
    // phrased as "you", every gap-check question asks about "them", and both
    // were assuming the user is the first party named -- which for a vendor
    // agreement made the system ask what the Buyer would have access to when the
    // user was the buyer asking about the vendor. A protection drafted for the
    // wrong side of the table is worse than no protection.
    //
    // Options are the document's own party labels, resolved at build time from
    // the naming policy, so this reads "Client / Consultant" on a consultancy
    // and "Landlord / Tenant" on a tenancy rather than "Party 1 / Party 2".
    drafting_for: {
      label: "Which side are you on?",
      type: "select",
      required: false,
      dynamicOptions: "party_roles",
      excludeShapes: ["NOTICE", "SWORN"],
      description:
        "The agreement is drafted even-handedly either way. This tells us whose risks to ask about, so the questions and the protections point at the right side of the table.",
    },
  ...
  DATA_PROCESSING_AGREEMENT: {
    ...
    // A data-processing agreement turns on these four answers, and the document
    // was drafting all of them the same way for every user. Seven of its eight
    // data-protection clauses were fully static text with no placeholder at all,
    // so two organisations with completely different processing arrangements
    // received byte-identical obligations.
    retention_period: {
      label: "How Long the Processor May Keep the Data",
      type: "text",
      required: false,
      description:
        "For example \"90 days after the engagement ends\". Section 8(7) of the DPDP Act, 2023 requires erasure once the purpose is served, so a stated period is what makes that obligation checkable.",
    },
    sub_processing_permitted: {
      label: "May the Processor Engage Sub-Processors?",
      type: "select",
      required: false,
      options: [
        "No — not without prior written consent",
        "Yes — with prior written notice and a right to object",
        "Yes — general authorisation, subject to the same obligations",
      ],
      description:
        "Whether your vendor may pass the data to its own vendors. Section 8(2) makes you responsible for the processor's acts, so this decides how far your exposure reaches.",
    },
    cross_border_transfer: {
      label: "Will Data Leave India?",
      type: "select",
      required: false,
      options: [
        "No — processed only in India",
        "Yes — to countries not restricted by the Central Government",
        "Not yet decided",
      ],
      description:
        "Section 16 of the DPDP Act, 2023 permits transfer outside India except to countries the Central Government restricts by notification.",
    },
    breach_notification_hours: {
      ...
      required: false,
      description:
        "How quickly the processor must tell you about a personal data breach, so that you can meet your own obligation to notify the Data Protection Board and affected Data Principals under Section 8(6).",
    },
  },
  ...
  IP_ASSIGNMENT_AGREEMENT: {
    // An assignment of software copyright and an assignment of a trade mark are
    // different instruments. The blueprint listed the copyright, patent, trade
    // mark and moral-rights clauses unconditionally, so every assignment carried
    // all four -- a codebase assignment arrived with a trade mark clause opening
    // "Where this Agreement assigns a trade mark", which is a clause apologising
    ...
    ip_types: {
      ...
      required: false,
      options: [
        "Copyright \u2014 software, content, designs or other works",
        "Patents or patent applications",
        "Trade marks and goodwill",
        "A mix of the above",
      ],
      description:
        "This decides which assignment clauses the deed carries. Copyright assignment must be in writing and signed under Section 19 of the Copyright Act, 1957; a trade mark assignment together with goodwill is recorded with the Registrar under Section 45 of the Trade Marks Act, 1999.",
    },
    contract_value: {
  ...
  CONSULTANCY_AGREEMENT: {
    // ── Objectives ────────────────────────────────────────────────────────
    // Every other field on this form asks for a FACT -- who, how much, when.
    // None asks what the user is trying to achieve or afraid of, which is why
    // 224 domain clauses average 1.5 appearances across the whole product:
    // nothing in the intake ever selects them. These two questions are the
    // intent layer. They drive clause SELECTION, not clause wording.
    protect_against: {
      label: "What should this agreement protect you from?",
      type: "multiselect",
      required: false,
      options: [
        "Missed deadlines or slow delivery",
        "Work not meeting the agreed standard",
        "Losing ownership of what I pay for",
        "Confidentiality breaches",
        "The consultant working for a competitor",
        "The consultant leaving mid-project",
        "Costs creeping beyond the quoted fee",
      ],
      description:
        "Pick everything that applies. Each one adds the clauses that deal with it; leaving one out keeps the document shorter.",
    },
    special_terms: {
      label: "Anything specific you want in this agreement?",
      type: "textarea",
      required: false,
      description:
        "Describe it in your own words, for example \"if they deliver faulty goods twice I want to walk away without notice\". It is recorded as a term of the agreement.",
    },
    party_1_name: {
      label: "Client Full Name / Company",
      type: "text",
  ...
  RENTAL_AGREEMENT: {
    ...
    // Two blueprints gated a clause on this variable and no form ever collected
    // it. Because an unanswered flag used to read as "false", every rental
    // agreement silently asserted that the tenant pays all utilities and that
    // arrears come out of the deposit -- a substantive allocation of cost that
    // nobody had agreed to. Asking is the fix; the gate is unchanged.
    utilities_included: {
      label: "Are utility charges included in the rent?",
      type: "select",
      required: false,
      options: ["Yes", "No"],
      description:
        "Answer No if the tenant pays electricity, water and gas separately on actual consumption. Answering No adds a clause making the tenant responsible for those bills and allowing arrears to be set off against the deposit.",
    },
    occupancy_term: {
  ...
  PRIVACY_POLICY: {
    // The retention clause said data is kept "only for as long as necessary" and
    // never said how long. Section 8(7) of the DPDP Act, 2023 requires erasure
    // once the purpose is served, and a stated period is what makes that
    // checkable rather than decorative.
    retention_period: {
      label: "How Long You Keep Personal Data",
      type: "text",
      required: false,
      description:
        "For example \"24 months after the account is closed\". Leave blank if retention varies by data type and you would rather state only the principle.",
    },
  ...
  TERMS_OF_SERVICE: {
    // A terms-of-service document varied on nothing. Seven of its clauses were
    // fully static, and the form asked only who the company was -- so a free
    // read-only site and a paid marketplace hosting user uploads received the
    // same terms, including a user-content licence for a service with no user
    // content and no payment terms for a service that charges.
    hosts_user_content: {
      label: "Do Users Post or Upload Content?",
      type: "select",
      required: false,
      options: ["No", "Yes"],
      description:
        "Reviews, listings, comments, photos, files. If yes, you need a licence to display what users post, and you take on intermediary due-diligence duties under the Information Technology (Intermediary Guidelines and Digital Media Ethics Code) Rules, 2021.",
    },
    requires_account: {
      label: "Do Users Create an Account?",
      type: "select",
      required: false,
      options: ["No \u2014 open access", "Yes \u2014 registration required"],
      description: "Whether the terms need account-security, suspension and closure provisions.",
    },
    minimum_age: {
      label: "Minimum Age to Use the Service",
      type: "select",
      required: false,
      options: [
        "18 and over only",
        "Under 18 permitted with parent or guardian consent",
      ],
      description:
        "A minor cannot contract under Section 11 of the Indian Contract Act, 1872, so terms accepted by one are void. Section 9 of the DPDP Act, 2023 additionally requires verifiable parental consent before processing a child's data, and bars tracking and targeted advertising directed at children.",
    },
    is_paid_service: {
      label: "Do Users Pay for the Service?",
      type: "select",
      required: false,
      options: [
        "No \u2014 free to use",
        "Yes \u2014 one-off or per-transaction charges",
        "Yes \u2014 recurring subscription",
      ],
      description:
        "A paid service needs fee, billing and cancellation terms. A recurring subscription taken by card additionally needs an e-mandate with pre-debit notification under the RBI framework for recurring transactions.",
    },
    company_name: { label: "Company / Business Name", type: "text", required: true, description: "The company's full registered name, exactly as on the certificate of incorporation.",},
    company_address: {
      label: "Registered Address",
...
VARIABLE_CONFIG.VENDOR_AGREEMENT = {
  ...VARIABLE_CONFIG.SUPPLY_AGREEMENT,
    // ── Objectives ────────────────────────────────────────────────────────
    // Every other field on this form asks for a FACT -- who, how much, when.
    // None asks what the user is trying to achieve or afraid of, which is why
    // 224 domain clauses average 1.5 appearances across the whole product:
    ...
    protect_against: {
      ...
      options: [
        ...
        "Confidentiality breaches",
        "Third-party or regulatory claims",
        "The vendor walking away early",
      ],
      description:
        "Pick everything that applies. Each one adds the clauses that deal with it; leaving one out keeps the document shorter.",
    },
